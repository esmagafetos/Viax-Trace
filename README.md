...
### GeocodeR BR — microserviço opcional (precisão máxima BR)

O instalador principal configura Node.js, PostgreSQL e a API. Para ativar o motor **GeocodeR BR** (fonte CNEFE/IBGE, precisão máxima para endereços brasileiros), execute o instalador standalone após a instalação principal:
...
> **Por que um instalador separado?** O pacote `r-base` foi removido do repositório oficial do Termux. O instalador usa **proot-distro** para criar um ambiente Ubuntu isolado dentro do Termux (sem root), instala **R 4.5** e baixa todos os pacotes (`arrow`, `duckdb`, `geocodebr`, `sf`, `plumber`) como **binários ARM64 pré-compilados via r-universe** — eliminando qualquer compilação C++. O processo leva **apenas 10–25 minutos**.
