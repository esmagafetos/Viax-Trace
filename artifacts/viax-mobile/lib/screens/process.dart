...
class _ProcessScreenState extends State<ProcessScreen> {
            ...
            child: Column(
              children: [
                ...
                if (processing) ...[
                  ...
                  // Contador de progresso — mesmo estilo dos steps, só texto
                  if (svc.hasProgress) ...[
                    const SizedBox(height: 4),
                    Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...
                        const SizedBox(width: 8),
                        Text(
                          '${svc.processed} / ${svc.total} endereços',
                          style: TextStyle(
                              fontSize: 12,
                              color: context.accent,
  ...
  Map<String, String>? _configWarning(SettingsProvider s) {
    if (s.instanceMode == 'googlemaps' && s.googleMapsApiKey.isEmpty) {
      return {
        'type': 'error',
        'message':
            'Motor Google Maps selecionado, mas nenhuma chave de API foi configurada.',
  ...
  Widget _detailRow(Map<String, dynamic> row, bool first) {
    ...
    final simStr =
        sim is num ? '${(sim * 100).toStringAsFixed(0)}%' : '—';
    return Container(
