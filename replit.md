...
3. **ViaX Mobile** (`artifacts/viax-mobile`) — Expo (React Native) Android app, brand-matched to web. **EAS-build-clean** (expo-doctor 17/17): pinned to SDK-54 versions `expo-file-system@~19.0.21`, `expo-image-picker@~17.0.10`, `expo-sharing@~14.0.8`, `@react-native-community/datetimepicker@8.4.4` to avoid duplicate native modules and "Major version mismatches" errors that previously blocked EAS APK builds.
   ...
   - **Wave 6 hotfix (mesmo dia):** o primeiro pacote de `expo-updates@^55` + `expo-keep-awake@^55` que instalei era a versão errada de SDK — `expo-doctor` apontou downgrade pra `expo-updates@~29.0.16` e `expo-keep-awake@~15.0.8` (compatíveis com Expo SDK 54). Corrigido antes do próximo CI build, agora `17/17 checks passed`.
   - **Wave 7 (Bloco A — pronto pra Play Store)** — destrava distribuição na Play Store sem mexer no produto:
     1. **`READ_EXTERNAL_STORAGE` removido do manifest** — irrelevante em SDK 33+ (que usa Storage Access Framework via `expo-document-picker`), só poluía a tela de permissões na Play Store. App agora pede só `INTERNET` + `ACCESS_NETWORK_STATE`.
     2. **`usesCleartextTraffic` virou condicional via `app.config.ts`** (substitui o `app.json` puro). Em build profile `development` libera HTTP em texto plano (necessário pra apontar pra `http://10.0.2.2:8080` no emulador ou IP de LAN); em `preview` e `production` força HTTPS-only — remove um achado clássico de pentest e satisfaz a política da Play Store. `versionCode` saiu do `app.json` também: o `eas.json` já tem `appVersionSource: "remote"` + `autoIncrement: true`, então o EAS é dono do número de build.
     3. **`@expo/ngrok` movido pra `devDependencies`** — só é usado pelo `expo start --tunnel`, não tem nada a fazer no bundle de produção.
     4. **`eas.json submit.production` configurado** com `serviceAccountKeyPath: ./google-play-service-account.json`, `track: internal`, `releaseStatus: draft`. `.gitignore` foi reforçado pra bloquear esse arquivo (e `*.keystore`, `*.jks`, `*.p8`, `*.p12`, `google-services.json`) — credenciais da Play Console nunca vão pro git.
     5. **Documentação completa de submit em PT-BR**:
        - `docs/privacy-policy-pt.md` — Política de Privacidade conforme LGPD (Lei 13.709/2018), com tabela de dados coletados, base legal por categoria, operadores (Sentry, EAS, Google Play), retenção, direitos do titular. Pronta pra hospedar em `https://viax.com.br/legal/privacidade`. Marcada com `(preencher)` nos campos que dependem de info real (e-mail oficial, DPO).
        - `docs/play-store-listing.md` — pacote completo de listing: nome do app (30 chars), descrição curta (76 chars), descrição completa, release notes da v1.2.0, especs de mídia, **Data Safety form com cada campo já mapeado** (a parte que mais reprova app na revisão), classificação IARC respondida pra dar **Livre**, roteiro sugerido de 5 screenshots, comandos prontos de `eas build` + `eas submit`, e cronograma de tracks (internal → closed → production só após Sentry estabilizar < 1% crash rate).
   - **Próximas Waves recomendadas (Bloco B + C)**: virtualizar listas longas com `FlatList` (history, resultados do Process), push notifications via `expo-notifications`, deep links de compartilhamento `/d/:analysisId`. Refatoração de arquivos grandes (`process.tsx` 1.103 linhas, `settings.tsx` 1.294) deliberadamente NÃO recomendada agora — alto custo, zero valor visível ao usuário. Fazer quando uma feature concreta exigir mexer neles, ou quando entrar segundo dev no projeto.
