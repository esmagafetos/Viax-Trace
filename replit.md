...
- **Nuance audit upgrade (Apr 30, 2026)**: Implemented 6 of 13 improvements from `docs/relatorios/analise-nuances-tamoios-23-04-2026.md` in `geocoder.ts` + `process.ts` (commit `ad44c6e`):
  - **M2** `toleranciaAdaptativa(parsed, base)` — rodovia 4×base, condomínio 50 m, comércio ≤80 m, composto +50 m, others = base. `verificarNuance` now uses `adjTolerance` for all distance thresholds.
  - **M5** Quadra/Lote parser (`extrairQuadraLote` + `QUADRA_LOTE_REGEX`) populates `parsed.quadra`, `parsed.lote` and triggers `is_condominio: true`.
  - **M8** `ENDERECO_COMPOSTO_REGEX` (servidão/beco/vila/conjunto/casa nº) sets `is_composto: true` so adaptive tolerance gives +50 m slack.
  - **M9** `process.ts` pre-pass dedupes paradas with identical GPS (5-decimal key) — clones the first decision into duplicates with `duplicata_de_linha` field, skipping redundant geocoder calls.
  - **M10** `process.ts` post-pass groups by normalized street name; pairwise distance ≥800 m flags `is_homonimo_intra_rota` and forces `is_nuance: true` with explanatory motivo.
  - **M11** `precisaoCoordenadaPorDecimais(lat, lon)` exposes coord precision (m) per row in `ResultRow.precisao_coord_m`. `tolerancia_aplicada_m` also surfaces for audit traceability.
  - All new fields are TypeScript-only additions to the JSON in `analyses.results` (no DB migration). `verificarNuance` signature preserved.
